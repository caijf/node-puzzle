...
PImage.decodeJPEGFromStream(stream).then(img => {
  const sx = 150;
  const sy = 50;
  ...
  puzzleCtx.clip();
  puzzleCtx.drawImage(img, sx, sy, PUZZLE_WIDTH, PUZZLE_HEIGHT, 0, sy, PUZZLE_WIDTH, PUZZLE_HEIGHT);
  ...
  return Promise.all([
    PImage.encodePNGToStream(puzzleCanvas, fs.createWriteStream(path.join(__dirname, 'draw-puzzle.png'))),
